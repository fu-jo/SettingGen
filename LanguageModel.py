...
class LanguageModel:
    ...
    def update(self):
        if self.n > 1: n_grams = tuple(ngrams(self.text, self.n))
        else: n_grams = tuple(self.text)
        if self.k > 1: k_grams = tuple(ngrams(self.text, self.k))
        else: k_grams = tuple(self.text)
        if self.n > self.k: n_grams = n_grams[1:]
        else: n_grams = n_grams[self.k:]
    ...
    def sample(self, condition: tuple) -> tuple:
        if type(condition) is not tuple: condition = tuple(condition)
        if self.cfd.get(condition) == None:
            raise KeyError
        rands = [tup for tup in self.cfd[condition].keys()]
        waits = [tup for tup in self.cfd[condition].values()]
        return random.choices(rands, weights=waits, k=1)

    def generate(self, starting_tokens: list, length: int, sep: str = ' ') -> str:
        gener = starting_tokens
        while len(gener) < length:
            if self.n != 1: gener = gener + [char for char in self.sample(gener[-self.k:])[0]]
            else: gener = gener + [char for char in self.sample(gener[-self.k:])]
